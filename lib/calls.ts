...
function tenantDirectory(tenantId: string) {
  return tenantId === DEFAULT_TENANT ? rootDirectory : path.join(rootDirectory, "tenants", tenantId);
}
function storePathFor(tenantId: string) { return path.join(tenantDirectory(tenantId), "calls.json"); }
const queues = new Map<string, Promise<unknown>>();
function queueFor(tenantId: string) { return queues.get(tenantId) || Promise.resolve(); }

async function readStore(tenantId = currentTenantId()): Promise<Store> {
  try { const parsed = JSON.parse(await readFile(storePathFor(tenantId), "utf8")) as Partial<Store>; return { contacts: parsed.contacts || [], messages: parsed.messages || [], calls: parsed.calls || [] }; }
  catch (error) { if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error; return { contacts: [], messages: [], calls: [] }; }
}
async function writeStore(store: Store) { const tenantId = currentTenantId(); const storePath = storePathFor(tenantId); await mkdir(tenantDirectory(tenantId), { recursive: true }); const temporary = `${storePath}.tmp`; await writeFile(temporary, JSON.stringify(store, null, 2), { encoding: "utf8", mode: 0o600 }); await rename(temporary, storePath); }
function mutate<T>(operation: (store: Store) => T | Promise<T>) { const tenantId = currentTenantId(); const run = queueFor(tenantId).catch(() => undefined).then(async () => { const store = await readStore(tenantId); const result = await operation(store); await writeStore(store); return result; }); queues.set(tenantId, run.then(() => undefined, () => undefined)); return run; }
function contactId(phone: string) { return `phone:${phone.trim() || "unknown"}`; }

export async function listContacts() { await queueFor(currentTenantId()); return (await readStore()).contacts.sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)); }
export async function getContact(id: string) { await queueFor(currentTenantId()); return (await readStore()).contacts.find((item) => item.id === id) || null; }
export async function listCallMessages(id: string) { await queueFor(currentTenantId()); return (await readStore()).messages.filter((item) => item.contactId === id); }
...
export async function listCallRecords() { await queueFor(currentTenantId()); return (await readStore()).calls.sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 200); }
export async function getCallRecord(id: string) { await queueFor(currentTenantId()); return (await readStore()).calls.find((item) => item.id === id) || null; }
export async function listMessagesSince(id: string, since: string) { await queueFor(currentTenantId()); return (await readStore()).messages.filter((item) => item.contactId === id && item.createdAt >= since); }
