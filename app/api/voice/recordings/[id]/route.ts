...
async function handleGET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  // И S3 key, и legacy-каталог общие для процесса. Сначала обязательно
  // подтверждаем call record внутри AsyncLocalStorage-контекста текущего tenant-а.
  if (!(await getCallRecord(id))) return new Response("Not found", { status: 404 });

  const range = request.headers.get("range") || undefined;
  try {
    const stored = await getRecordingObject(currentTenantId(), id, range);
    if (stored) {
    ...
    const local = await localRecording(path.join(recordingsDirectory, `${id.toLowerCase()}.wav`), range || null);
