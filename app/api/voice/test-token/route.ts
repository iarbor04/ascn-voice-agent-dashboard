...
async function handlePOST(request: Request) {
  ...
  const secret = process.env.BROWSER_TOKEN_SECRET?.trim();
  if (!secret) return Response.json({ error: "BROWSER_TOKEN_SECRET не настроен" }, { status: 503 });
  ...
  const payload = `${agent.id}.${currentTenantId()}.${expiresAt}`;
